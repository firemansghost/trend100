...
- **Repair EOD Internal Gaps (manual):** Restores production EOD LKG (`marketstack-eod-v2`), fails closed on cache miss, requires SPY last date within 10 UTC days, then runs `audit-marketstack-eod-gaps` against the **SPY usable-session calendar** from `start_date` (default 2026-02-18) through latest SPY. Long gap = ≥5 consecutive missing SPY sessions (isolated single misses are not auto-repair candidates). **Audit-only (`repair=false`): zero Marketstack/Stooq.** `repair=true`: print candidate count and projected ticker units (≈ candidate count for this bounded window), fetch only those symbols, two-phase in-memory validate then merge. Post-repair audit must have **zero** long-gap candidates. Saves EOD LKG only on repair success. Does not deploy and does not rebuild health-history. **Do not rerun Backfill Health History integrity repair until this EOD repair passes.**
