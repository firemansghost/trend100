...
- **Repair EOD Internal Gaps (manual):** Restores production EOD LKG (`marketstack-eod-v2`), fails closed on cache miss, requires SPY last date within 10 UTC days, then runs `audit-marketstack-eod-gaps` against the **SPY usable-session calendar** from `start_date` (default 2026-02-18) through latest SPY. Long gap = ≥5 consecutive missing SPY sessions (isolated single misses including 2026-06-04 are reported, not auto-repaired, not fabricated). **Audit-only:** leave `repair` unchecked; leave `confirm_repair` blank; leave `max_ticker_units` at `0`. Zero Marketstack/Stooq. **Repair UI procedure:** (1) Actions → **Repair EOD Internal Gaps** → Run workflow. (2) Check **`repair`**. (3) Set **`confirm_repair`** to exactly `REPAIR` (case-sensitive). (4) Set **`max_ticker_units`** to a positive integer **≥** that run’s audited long-gap **candidate count** (not a hardcoded workflow constant). (5) Keep `start_date` `2026-02-18` unless intentionally changing the window. (6) Run. If `repair` is checked but confirmation is blank, or candidates exceed the cap, the job fails **before** Marketstack. After LKG `Linux-marketstack-eod-v2-32885511375`, the next repair’s candidate set should naturally be the remaining long-gap names (historically FBTC/FETH/SLB); set `max_ticker_units` to that audited count. Repair prints quota then `actualTickerUnits`. Phase A: **RESOLVED** / **PROVIDER_LIMITED** / **UNRESOLVED**. Phase B merges RESOLVED + PROVIDER_LIMITED real bars only (no synthetic data). Live provider-limited residuals are recorded in `.provider-gap-residuals-v1.meta` inside the EOD Actions cache. Completeness `--fail-on-unverified-long-gap` is green when remaining long gaps are provider-confirmed; `symbolsWithLongGap` still reports the real count. Provider residual evidence applies to the **recorded historical long-gap ranges**, not the frozen `auditEnd` of the run that observed them: a later SPY tip does not expire an unchanged residual. Isolated <5-session misses outside a recorded residual do not invalidate it; any new or expanded ≥5-session gap remains unverified and blocking. `auditStart`/`auditEnd` stay in metadata as provenance. Does not deploy. Does not rebuild health-history. **Do not run Backfill Health History integrity repair until `symbolsWithUnverifiedLongGap = 0`.** `--fail-on-suspicious` stays the health-history correctness gate. Run **32885511375**: 144/147 merged, LKG saved, red only on the old completeness gate for FBTC/FETH/SLB (105→10 sessions `2026-05-26..2026-06-08`).
...
- **Backfill Health History (manual, provider-free):** Restores production EOD (`marketstack-eod-v2`) and health-history LKG. **Fails closed** if `cache-matched-key` is empty (no repo EOD fallback) or if SPY last date is >10 UTC calendar days old. **No** `MARKETSTACK_API_KEY`, **no** Stooq. `MARKETSTACK_OFFLINE=1`. `repair_health_history_integrity=true` first runs provider-free `audit-marketstack-eod-gaps --fail-on-unverified-long-gap` (prints provider-limited residuals; refuses Pass A/B if `symbolsWithUnverifiedLongGap > 0`). Then Pass A (2026-02-18→today, all decks) then Pass B (MACRO variants only, 2019-10-01→today), then `report-health-history-integrity --fail-on-suspicious` (exit 1 if suspicious stale UNKNOWN > 0 → LKG not saved). Each variant repair calendar is **variant EOD dates ∪ existing in-range history dates** (no invented weekdays; scoring/`minEligible`/`minKnownPct` unchanged). Run **32890542447**: EOD gate passed; Pass A recovered ~120–130 recent dates; Pass B left **21** suspicious rows (7 uranium / 7 crypto / 7 dollar = history total − recomputed dates); LKG not saved. Integrity report prints each suspicious row (`date known= eligible= total=`). Honest UNKNOWN during a provider-limited hole (e.g. FBTC+FETH with eligible=0) must not be fabricated. Ordinary backfill reports only (non-strict). Verify with `pnpm verify:artifacts -- --health-history-only` (does **not** check Turbulence/Green Bar/snapshot.PLUMBING/War Lie Detector). Production `pnpm verify:artifacts` remains full-strength. Does **not** deploy. After repair, logs per MACRO section (FX/Metals/Commodities/Energy/Uranium/Crypto/Dollar): total/valid/UNKNOWN, first, first valid, last, longest gap after 2026-02-18, `suspiciousStaleUnknown` count (UNKNOWN with eligible≥min(10,n) and known/eligible≥0.7). Expect **0** suspicious rows after Pass B once existing in-range rows are actually revisited; FX/Metals/Commodities first-valid near 2020-09/10; Dollar/Energy later; Uranium after 2021 inception; Crypto keeps substantial legitimate UNKNOWN. Do not treat exact first-valid dates as permanent invariants. Wait for the next automatic Vercel Prebuilt Prod run after merge **before** dispatching repair, so EOD LKG and an initial health-history LKG exist. Do not dispatch Daily Artifacts solely to publish the repair unless explicitly needed.
