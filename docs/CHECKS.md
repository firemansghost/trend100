...
- **Turbulence shock:** Validates `public/turbulence.shock.json`:
  ...
  - `update:turbulence-shock` logs: union vs qualified calendar counts, discarded dates since 2023 (date, nCloses, spyClose), latest shock row (`date`, `nAssets`, `nPairs`, `shockRaw`, `shockZ`). Calendar = SPY close + ≥8 recent-universe closes **with finite close > 0**. shockRaw still uses floor-6 acceptance until a later PR. Temporary eligibility diagnostics remain. **Post-deploy (after non-positive close rejection):** expect 2026-06-04 zero bars **not** on the shock calendar; no ±Infinity returns; latest shock date near **2026-08-24**; no 52-row trailing-null trim; `verify:artifacts` should pass. Latest `nAssets` should recover substantially (do not claim 12 until CI shows it). Cache logs may show `[cache] <SYM>: ignored N invalid EOD bar(s): 2026-06-04`. Load rewrites the sanitized file immediately so `cache/save` persists the clean JSON even if Marketstack latest is still 2026-08-24.
