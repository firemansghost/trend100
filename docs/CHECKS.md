...
- **Plumbing War Lie Detector:** Validates `public/plumbing.war_lie_detector.json`. Regime is bucket-based (PR25): plumbing low → THEATER; plumbing strong + (substitution OR gold) → REAL_RISK; else WATCH. UI (PR26) displays CONTAINED instead of THEATER; bucket chips (Plumbing / Substitution / Macro) shown when `bucketState` present or derived.
