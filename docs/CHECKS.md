...
- **Turbulence shock:** Validates `public/turbulence.shock.json`:
  ...
  - `update:turbulence-shock` logs: union vs qualified calendar counts, discarded dates since 2023 (date, nCloses, spyClose), latest shock row (`date`, `nAssets`, `nPairs`, `shockRaw`, `shockZ`). Calendar = SPY close + ≥8 recent-universe closes. shockRaw still uses floor-6 acceptance until a later PR.
