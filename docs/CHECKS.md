...
- **CI cache: turbulence gates LKG (rolling):** Both `daily-artifacts-deploy.yml` and `vercel-prebuilt-prod.yml` restore `public/turbulence.gates.json` before the production URL prefetch (`turbulence-gates-lkg-v1` prefix), then save the file after `artifacts:refresh` when it exists (same per-run key pattern as EOD). **`vercel-prebuilt-prod.yml`** uses a **safe prefetch**: curl writes to a temp path first; a **non-empty** JSON **array** is copied into `public/` only on success (failed or invalid prefetch does not clobber a restored file). Logs `restored_file_before_prefetch`, curl result, temp file presence, temp parse/array/non-empty, and `final_*` stats. **`daily-artifacts-deploy.yml`** still prefetches directly to `public/` (see DECISIONS if aligned later). Bump `lkg-v1`→`lkg-v2` if the cache entry must be invalidated.
