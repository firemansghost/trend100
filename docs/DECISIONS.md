...
`repair=true` now requires `confirm_repair=REPAIR` exactly and a positive integer `max_ticker_units` with `candidateCount <= max_ticker_units` **before any Marketstack call**. Audit-only needs neither field. Fetch still uses one bounded `fetchEodSeriesRange` per long-gap candidate. Phase A classifies **RESOLVED** (`longestMissingRun < 5`), **PROVIDER_LIMITED** (valid fetch that shrinks a large hole but leaves `longestMissingRun >= 5`), or **UNRESOLVED** (empty/truncated/no overlap/no improvement). Phase B merges **RESOLVED + PROVIDER_LIMITED** real bars only. Unresolved files are untouched. **No synthetic/forward-fill/proxy closes.**

Run **32885511375** restored `Linux-marketstack-eod-v2-32884878728`, confirmed quota (`REPAIR` / max **147**), spent **147** ticker-units, classified **144 RESOLVED** and **3 UNRESOLVED** under the old two-way rule (FBTC/FETH/SLB each `preLongest=105` → simulated `post=10` on `2026-05-26..2026-06-08`). Phase B merged the 144; post-repair audit: **84** complete, **76** with missing sessions, **3** long-gap, most remaining misses isolated (`2026-06-04`, longest=1). New EOD LKG **`Linux-marketstack-eod-v2-32885511375` was saved**. The job was red only because `--fail-on-long-gap` saw FBTC/FETH/SLB. Those three are now **PROVIDER_LIMITED**: persist the real shrink from 105→10 and record live-fetch evidence in `data/marketstack/eod/.provider-gap-residuals-v1.meta` (not counted as a symbol JSON). Completeness uses `--fail-on-unverified-long-gap` (green when remaining long gaps are backed by that metadata). Matching uses **current long-gap ranges only** (`sessions >= 5`) against **individual** recorded residual ranges; a later SPY `auditEnd` (or an isolated <5-session miss outside the residual) does not expire evidence. A new/expanded ≥5-session gap is unverified. Metadata `auditStart`/`auditEnd` remain provenance of when the provider was queried. Audit still reports `symbolsWithLongGap` honestly (do not pretend it is 0). Health-history integrity repair may proceed when `symbolsWithUnverifiedLongGap=0`; `--fail-on-suspicious` is unchanged. Honest UNKNOWN during a provider-limited crypto hole (eligible=0) must not be fabricated away.

**Why:** Backfill Health History **32879333727** restored a current SPY tip (`2026-08-24`) and 160 EOD files, but Pass A found only ~20 trading days for US_FACTORS / GLOBAL / FIXED_INCOME / most MACRO subsections / METALS_MINING versus ~126–128 for LEADERSHIP / US_SECTORS / MACRO.energy / PLUMBING. MACRO subsection longest gap after 2026-02-18 was **160d (2026-02-18 → 2026-07-28)** except Energy (~4d). Pass B cleared most stale UNKNOWN policy rows but **27** suspicious Uranium/Crypto/Dollar rows remained, so LKG was correctly not saved. Same internal-hole class as the earlier Turbulence sector caches. Health-history `--fail-on-suspicious` stays strict.
...
**Why:** Production previously **did not** persist health-history between runs. Each deploy started from git-tracked histories, **upserted only the current `asOfDate`**, deployed, and discarded the rest. That produced (1) a dense repo-era tail through ~**2026-02-17** then a jump to the latest deploy date, and (2) MACRO section UNKNOWN rows from the first section backfill under **uncapped `minEligible=10`** (fixed 2026-02-15 in `584bac0`) that daily upsert never recomputed. Commit `92557e9` regenerated MACRO sections after that fix, but without an LKG cache the repair was not durable. Run **32890542447** restored EOD `Linux-marketstack-eod-v2-32890124260` and health-history `Linux-health-history-lkg-v1-32889013060`. The EOD continuity gate passed (`symbolsWithLongGap=3`, all provider-limited FBTC/FETH/SLB, `symbolsWithUnverifiedLongGap=0`). Pass A recovered normal recent-date counts (~120–130). Pass B dropped suspicious UNKNOWN from 27 to **21** (exactly 7 each in uranium/crypto/dollar: final total minus recomputed dates, e.g. 1272−1265). LKG was not saved. Root cause: backfill dates came only from the variant EOD cache, so existing history rows absent from that EOD set were never recomputed. No extra Marketstack fetch is required.

**Repair path:** Manual **Backfill Health History** restores the **same** production EOD cache (`marketstack-eod-v2`) and health-history LKG, runs **offline** (`MARKETSTACK_OFFLINE=1`, no API keys, no Stooq). It **fails closed** if no EOD Actions cache key matches (never uses stale repo EOD) and if restored **SPY** last date is more than **10 UTC calendar days** old. Input `repair_health_history_integrity` runs Pass A (`2026-02-18`→UTC today, all decks) then Pass B (`2019-10-01`→today, `--deck MACRO --variants-only`). After Pass B, `--fail-on-suspicious` exits non-zero if any MACRO variant row is UNKNOWN while already meeting current eligible/knownPct rules; LKG is saved only on `success()`. Each variant’s backfill calendar is the union of **active-variant EOD dates** and **existing in-range health-history dates** (weekends excluded; no invented weekdays or EOD bars), so a stale UNKNOWN cannot survive solely because it is missing from that variant’s EOD date set. Verification is `pnpm verify:artifacts -- --health-history-only` (health-history files only). Production `pnpm verify:artifacts` remains the full suite. The backfill job **does not deploy**. The next normal Prebuilt/Daily run restores the repaired LKG, refreshes current EOD, verifies, deploys, and saves the next LKG.
